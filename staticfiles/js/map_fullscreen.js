...
document.addEventListener("DOMContentLoaded", function () {
    let map = L.map('map3').setView([14.6928, -17.4467], 10);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; OpenStreetMap contributors'
    }).addTo(map);

    let geojsonLayer; // Stocker la couche GeoJSON

    document.getElementById("upload-form").addEventListener("submit", function (event) {
        event.preventDefault(); // ✅ Empêcher le rechargement de la page

        let formData = new FormData(this); // Récupérer les fichiers du formulaire

        fetch("/upload/", {  // ✅ URL correcte vers la vue Django
            method: "POST",
            body: formData,
            headers: {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // ✅ Affichage du GeoJSON sur la carte
                geojsonLayer = L.geoJSON(data.geojson, {
                    style: { color: "blue", weight: 2 }
                }).addTo(map);
                map.fitBounds(geojsonLayer.getBounds());
            } else {
                Swal.fire("Erreur", data.error || "Le fichier est invalide.", "error");
            }
        })
        .catch(error => {
            console.error("Erreur lors de l'importation :", error);
            Swal.fire("Erreur", "Impossible d'importer le fichier.", "error");
        });
    });
});
